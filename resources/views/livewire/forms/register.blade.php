<div style="width: fit-content;height: fit-content;margin: auto ;background-color: gainsboro; padding: 40px;">
    ...
    <form wire:submit.prevent="register"> <!-- Gunakan wire:submit.prevent -->
        @csrf
        <!-- NIM -->
        <div>
        ...
        <!-- Role Selection -->
        <div style="margin-top: 1rem;">
            <label for="role" style="display: block; font-weight: 500; font-size: 0.875rem; color: #374151;">Role</label>
            <select
                wire:model="login_as"
                id="login_as"
                style="display: block; margin-top: 0.25rem; width: 100%; padding: 0.5rem; border: 1px solid #D1D5DB; border-radius: 0.375rem; background-color: white;">
                <option value="">Select Role</option>
                <option value="1">Bendahara</option>
                <option value="2">Serving</option>
            </select>
            @error('login_as') <span style="color: red; font-size: small">{{ $message }}</span> @enderror
        </div>
